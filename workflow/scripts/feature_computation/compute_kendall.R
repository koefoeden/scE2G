...
kendall_mutliple_genes <- function(bed.E2G,
  ...
  bed.E2G.output <- foreach(
    gene.name = unique(mcols(bed.E2G.filter)[, colname.gene_name]),
    .combine = "c"
  ) %do% {
...
kendall_multiple_genes_parallel <- function(bed.E2G, data.RNA, data.ATAC, colname.gene_name, colname.enhancer_name, colname.output, n_workers = 120) {
  ...
  bed.E2G.output <- BiocParallel::bplapply(genes, function(gene.name) {
    # Filter RNA matrix
    RNA_matrix_filtered <- data.RNA[gene.name, ] %>% as.numeric()

    # Filter ATAC matrix
    gene_idxs <- bed.E2G.filter %>%
      GenomicRanges::mcols() %>%
      .[, colname.gene_name] == gene.name
    E2G_pairs_filtered_GRanges <- bed.E2G.filter[gene_idxs]
    peak_names <- E2G_pairs_filtered_GRanges %>%
      mcols() %>%
      .[, colname.enhancer_name]
  ...
  reduced_results <- purrr::reduce(bed.E2G.output, `c`)
...
df.exp_inf <- data.frame(
  mean_log_normalized_rna = rowMeans(matrix.rna),
